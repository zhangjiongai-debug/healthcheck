"""健康检查结果数据结构与报告输出 (与 k8s/keycloak 模块共用相同设计)。"""
...
def print_report(groups: list[CheckGroup], *, verbose: bool = False):
    ...
    if worst == Severity.OK:
        print(f"  {_COLORS[Severity.OK]}PostgreSQL 健康状态良好{_RESET}\n")
    elif worst == Severity.WARN:
        print(f"  {_COLORS[Severity.WARN]}PostgreSQL 存在告警项，建议关注{_RESET}\n")
    else:
        print(f"  {_COLORS[Severity.ERROR]}PostgreSQL 存在异常，请尽快处理{_RESET}\n")
