...
def print_report(groups: list[CheckGroup], *, verbose: bool = False):
    ...
    if worst == Severity.OK:
        print(f"  {_COLORS[Severity.OK]}GitLab 健康状态良好{_RESET}\n")
    elif worst == Severity.WARN:
        print(f"  {_COLORS[Severity.WARN]}GitLab 存在告警项，建议关注{_RESET}\n")
    else:
        print(f"  {_COLORS[Severity.ERROR]}GitLab 存在异常，请尽快处理{_RESET}\n")
