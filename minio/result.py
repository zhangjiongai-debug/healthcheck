"""健康检查结果数据结构与报告输出 (与 k8s/keycloak/postgresql 模块共用相同设计)。"""
...
def print_report(groups: list[CheckGroup], *, verbose: bool = False):
    ...
    if worst == Severity.OK:
        print(f"  {_COLORS[Severity.OK]}MinIO 健康状态良好{_RESET}\n")
    elif worst == Severity.WARN:
        print(f"  {_COLORS[Severity.WARN]}MinIO 存在告警项，建议关注{_RESET}\n")
    else:
        print(f"  {_COLORS[Severity.ERROR]}MinIO 存在异常，请尽快处理{_RESET}\n")
